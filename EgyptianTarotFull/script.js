...
const userNumCards = document.getElementById('userNumCards');
const cardsList = document.getElementById('cardsList');
const tiradaList = document.getElementById('tiradaTres');
...
btn.addEventListener('click', drawTarot);

function drawTarot() {
  // Read how many cards the user wants
  const numCards = parseInt(userNumCards.value, 10);

  // Validate input
  if (numCards > 0 && numCards <= maxCards) {
    drawCards(numCards);
  } else {
    swal({ 
      text: `There are ${maxCards} cards in the Tarot. Please choose a value up to ${maxCards}. Recommended spreads are 3, 5, or 10 cards.` 
    });
  }
}

function drawCards(num) {
  tiradaList.innerHTML = ''; // Clear existing cards
  let availableCards = [...cardData]; // Create a copy of the full deck

  if (num > availableCards.length) {
    swal({ 
      text: `You requested ${num} cards, but only ${availableCards.length} are available. Please choose a smaller number.` 
    });
    return;
  }

  let drawn = []; // Track drawn cards
...
availableCards.splice(randomIndex, 1);
  ...
  count++;
  console.log(`Draw #${count}, card IDs: ${drawn.join(', ')}`);
}

// Utility function for random integer in [min, max]
function getRandomIntInclusive(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}

// Display all 78 cards on screen
