...
class Game {
  ...
  resizeCanvas() {
    ...
    // Factor in devicePixelRatio (capped at 2) so the backing store matches the
    // physical pixels on Retina / iPad displays for crisp pixel art.
    const dpr = Math.min(2, window.devicePixelRatio || 1);
    // Pick an integer scale of VW=480 that matches or exceeds the physical size, capped at 4x.
    const targetW = Math.min(1920, Math.max(VW, Math.ceil(cssW * dpr)));
