...
const cardData = [
  ...
  { id: 3, name: "The Empress", description: "The Empress represents femininity, beauty, nature, and abundance.", imagePath: "./images/EMPRESS.PNG", fileName: "./full_descriptions/4_3TheEmperessMajorArcana.txt" },
...
document.addEventListener("DOMContentLoaded", () => {
  const deck = [...cardData]; // Clone cardData for shuffling
  let deckArr = [...Array(deck.length).keys()]; // Array of indices
  let drawnCards = []; // Stores drawn cards in order
  let backImg = "<img class='img-thumbnail' src='./images/thoth-back.jpeg' />";
  const numCards = 6;

  // Initialize deck display
  for (let i = 1; i <= numCards; i++) {
    document.getElementById(`position-${i}`).innerHTML = backImg;
  }

  // Reset deck & tracking
  function resetDeck() {
    deckArr = [...Array(deck.length).keys()];
    drawnCards = [];
  }

  // Random Card Generator
  function drawCard() {
    if (deckArr.length === 0) resetDeck();
    const randIndex = Math.floor(Math.random() * deckArr.length);
    return deckArr.splice(randIndex, 1)[0]; // Remove & return card index
  }

  // Fetch Full Description from File
  async function fetchFullDescription(filePath) {
    try {
...
function changeBackground() {
  document.body.style.background = "linear-gradient(to bottom, black, white)";
  document.body.style.backgroundAttachment = "fixed";
}

// Tarot Spread Function
async function cardSpread() {
  drawnCards = []; // Clear previous draw

  // Draw 6 cards and store them in the array
  for (let i = 0; i < numCards; i++) {
    const cardIndex = drawCard();
    const card = deck[cardIndex];
    drawnCards.push(card); // Maintain order
  }

  // Display images in drawn order
  drawnCards.forEach((card, index) => {
    document.getElementById(`position-${index + 1}`).innerHTML = `
      <img class='img-thumbnail' src='${card.imagePath}' alt='${card.name}'>
    `;
  });

  // Change background after drawing
  changeBackground();

  // Fetch descriptions in the corrected order
  await displayDescriptions();
}

// Assign event listener
document.getElementById("reading-btn").addEventListener("click", cardSpread);

  // Generate Descriptions in Correct Order
  async function displayDescriptions() {
    let descriptionsHtml = "";
    let descriptions = [];

    // Fetch all descriptions in order
    for (let i = 0; i < drawnCards.length; i++) {
      const card = drawnCards[i];
      const fullDescription = await fetchFullDescription(card.fileName);
    ...
    // Move **5th description** to **1st position**
    if (descriptions.length >= 5) {
      let fifthDescription = descriptions.splice(4, 1)[0]; // Remove 5th description
      descriptions.unshift(fifthDescription); // Insert it at position 1
    }

    // Generate the HTML for descriptions
    descriptions.forEach((desc, index) => {
      descriptionsHtml += `
        <div class="card-description">
          <h3>Position ${index + 1}: ${desc.card.name}</h3>
          <img class='description-img' src='${desc.card.imagePath}' alt='${desc.card.name}'>
          <p><strong>Short Description:</strong> ${desc.card.description}</p>
          <div class="full-description">
            <p><strong>Full Description:</strong></p>
            <div class="description-scroll">${desc.fullDescription}</div>
          </div>
        </div>
      `;
    });

    document.getElementById("description-container").innerHTML = descriptionsHtml;
    document.getElementById("reading-btn").innerText = "Another Reading?";
  }
