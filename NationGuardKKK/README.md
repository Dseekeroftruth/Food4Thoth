---
title: "Manufactured Emergencies & Militarized Repression README"
subtitle: "Why Deploying the National Guard to Washington, D.C. on False Pretenses is Unconstitutional and Historically Resonant with State Terror"
author: DeJahn (Food4Thoth)
created: 2025-07-20
last_updated: 2025-07-20
permalink: /NationGuardKKK/readme/
image: https://www.food4thoth.com/NationGuardKKK/og-preview.jpg
tags: [National Guard, Posse Comitatus, Insurrection Act, constitutional law, militarization, KKK, Jim Crow, COINTELPRO, D.C., state terror, Food4Thoth]
description: |
  A constitutional, historical, and doctrinal analysis demonstrating that the 2025 deployment of National Guard units to Washington, D.C. on fabricated “crime emergency” grounds is unlawful and unconstitutional. The work situates the deployment within a lineage of state terror—from the KKK and Jim Crow through COINTELPRO to modern carceral expansion—and offers concrete policy reforms.
layout: default
---

<style>
  body { background-color: #000; color: #fff; }
  a { color: #66e3ff; }
  h1, h2, h3, h4, h5, h6 { color: #ffeb3b; }
  .nav-button {
    display: inline-block; padding: 10px 18px; margin: 6px;
    background: linear-gradient(135deg, #ff00ff, #001eff);
    color: #fff; font-weight: bold; text-decoration: none; border-radius: 8px;
    box-shadow: 3px 3px 12px rgba(255,0,255,.5); font-size: 1rem; transition: all .3s ease;
  }
  .nav-button:hover { background: linear-gradient(135deg, #00ffff, #6600cc); box-shadow: 0 0 10px #00ffff; }
  .wrap { word-wrap: break-word; overflow-wrap: break-word; white-space: normal; }
</style>

# Manufactured Emergencies & Militarized Repression

*Why Deploying the National Guard to Washington, D.C. on False Pretenses is Unconstitutional and Historically Resonant with State Terror*

<div>
  <a class="nav-button" href="/NationGuardKKK/index.html">Open Article</a>
  <a class="nav-button" href="/NationGuardKKK/ManufacteredEmergencies.pdf" target="_blank" rel="noopener">Download PDF</a>
  <a class="nav-button" href="https://mozilla.github.io/pdf.js/web/viewer.html?file=https://www.food4thoth.com/NationGuardKKK/ManufacteredEmergencies.pdf" target="_blank" rel="noopener">View PDF in Browser</a>
</div>

---

## 📜 Overview

This project argues that the **2025 National Guard deployment to Washington, D.C.**—justified by a manufactured “crime emergency”—violates the Constitution and existing federal law.  
Through constitutional doctrine, case law, and historical parallels, it demonstrates how **fabricated crises** enable **executive overreach** and reproduce a lineage of **state-sanctioned repression**, from the **Ku Klux Klan** and **Jim Crow** to **COINTELPRO** and contemporary detention regimes.

It advances four core claims:

- **Unconstitutional** under separation of powers, the **Take Care Clause**, the **Posse Comitatus Act**, and constraints of the **Insurrection Act**.  
- **Ultra vires** because **fabricated emergencies** cannot create extraordinary executive power.  
- **Historically continuous** with militarized repression against marginalized communities.  
- **Ethical duty**: Guard members must refuse manifestly unlawful orders.

---

## 🔍 Features

- **Interactive Table of Contents** (in-article) generated from `h2/h3` headings.  
- **Modal PDF viewer** for the full document.  
- **Smooth scrolling + anchor offset** so headings aren’t hidden under sticky elements.  
- **Footnote popovers** for inline citation preview.  
- **Mobile-first accessibility** with keyboard and escape-close support for modals.  
- **Performance**: `preload`/`prefetch` for assets, PDF, and navigation.

---

## 📑 Table of Contents (Article)

- [Introduction](../index.html#intro)  
- [I. Constitutional Framework](../index.html#i-framework)  
  - [A. Separation of Powers](../index.html#i-a)  
  - [B. Due Process & Equal Protection](../index.html#i-b)
- [II. Illegality of False Pretenses](../index.html#ii-illegality)  
  - [A. Constitutional Fraud](../index.html#ii-a)  
  - [B. Take Care Clause & Ultra Vires](../index.html#ii-b)
- [III. Historical Parallels: From the Klan to ICE](../index.html#iii-parallels)  
  - [A. Klan & State Terror](../index.html#iii-a)  
  - [B. Jim Crow Legal Fictions](../index.html#iii-b)  
  - [C. Modern Carceral Continuities](../index.html#iii-c)
- [IV. Case Studies of Manufactured Emergencies](../index.html#iv-cases)  
  - [A. Bonus Army](../index.html#iv-a)  
  - [B. Kent State](../index.html#iv-b)  
  - [C. COINTELPRO](../index.html#iv-c)
- [Part V: Doctrinal Analysis](../index.html#v-doctrine)  
  - [Milligan](../index.html#v-a) · [Duncan](../index.html#v-b) · [Youngstown](../index.html#v-c) · [Hamdi](../index.html#v-d)
- [Part VI: Policy Reform Recommendations](../index.html#vi-policy)  
- [VII. Conclusion](../index.html#vii-conclusion)  
- [Appendices](../index.html#appendix)  
- [Legal Disclaimer](../index.html#legal-disclaimer)

---

## 🛠️ Technologies Used

- **HTML5 / CSS3** for dark theme, responsive layout, and accessible typography.  
- **Vanilla JavaScript** for:
  - TOC generation  
  - Smooth scrolling with `scroll-margin-top` offset  
  - Modal open/close + focus management  
  - Scroll-to-top/bottom controls  
- **Perf-friendly hints**: `rel=preload` / `rel=prefetch` for images, CSS, and PDF.

---

## 🎯 Highlights & Takeaways

- **Youngstown** places the executive at its **“lowest ebb”** when acting against Congress; fabricated emergencies cannot bypass the PCA/Insurrection Act.  
- **Milligan** and **Duncan** limit military authority where civil courts are open and no imminent public danger exists.  
- **Hamdi** affirms due process even in genuine emergencies; fabricated ones doubly fail.  
- Historical **manufactured crises** (Bonus Army, Kent State, COINTELPRO) show how **false threat narratives** rationalize repression.

---

## 📚 Citation

> DeJahn. “Manufactured Emergencies and Militarized Repression — Why Deploying the National Guard to Washington, D.C. on False Pretenses is Unconstitutional and Historically Resonant with State Terror.” *Food4Thoth* (2025). https://www.food4thoth.com/NationalGuardKKK/

---

## 🙏 Acknowledgments

Gratitude to **survivors, journalists, legal scholars, and community researchers** whose testimony, data, and analysis inform this work, and to the **Food4Thoth collective** for editorial support and hosting.

---

## License

This project is released under a  
[Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International License](https://creativecommons.org/licenses/by-nc-sa/4.0/).

---

## Further Reading

- Visit the hub: [Food4Thoth](../index.html)  
- Contact: [food4thoth@proton.me](mailto:food4thoth@proton.me)

---

## ❤️ Credits & Attribution

Created by **DeJahn** under the **Food4Thoth** platform.  
All content and design decisions are original and community-curated.

---

# 🌌 Philosophy and Vision

Food4Thoth is inspired by Thoth’s principles:
- **Creativity** · **Exploration** · **Community** · **Playfulness**

A digital garden where rigorous inquiry meets accessible design.

---

# ✨ Why Visit Food4Thoth?

1. **Diverse offerings**: law, history, culture, mysticism.  
2. **Interactive tools**: playful apps alongside research.  
3. **Community projects**: collaborations, gardens, and open archives.  
4. **Inspiration**: a home for curiosity and civic imagination.

---

## 🤝 Support and Contributions

Your support sustains research, tools, and community projects.

### Donation Options

#### Traditional:
1. [PayPal](https://paypal.me/artabillies)  
2. [Venmo](https://venmo.com/u/DeJahnvu)

#### Cryptocurrency:
- **Ethereum (ETH) & ERC-20**:  
  <div class="wrap">0x900e8f0d397048fD946b05553DeD5Ed3D5e4f1a0</div>

- **Bitcoin (BTC)**:  
  <div class="wrap">bc1qcsa7ffef296pp9hkrn03p9wu7lt0fm3s2sz0wp</div>

- **Ethereum Classic (ETC)**:  
  <div class="wrap">0xEb3C0e08868ACB0f515442579333c41E7a34F215</div>

- **Solana (SOL)**:  
  <div class="wrap">B7nCFQs6HkFAvkz1wEUiPpM4Cj7G6FJNYQ7Avrt6a4cm</div>

- **Ripple (XRP)**:  
  Address: <div class="wrap">rEAKseZ7yNgaDuxH74PkqB12cVWohpi7R6</div>  
  Memo: `3109966062`

- **Dogecoin (DOGE)**:  
  <div class="wrap">DP2e6J8NbUzswLtBw8ou2xYz4BinyzgU7n</div>

- **Cardano (ADA)**:  
  <div class="wrap">addr1qxqgjp4h4vh4pxrg7jur8m96lzf5w98cahfflrw376qhufgg6h5us0avc20ee2azzun58lgylyl54sjr6y9efwq86krs3ladtw</div>

- **Bitcoin Cash (BCH)**:  
  <div class="wrap">bitcoincash:qpu93py8j8ykcf7m6tmau2hldefl67t9lydw8afsa5</div>

- **Stellar (XLM)**:  
  Address: <div class="wrap">GB2ES2N326MZK4EGJBKN3ZARCQ5RTFQSAWIJAAKFVIIIJSCC35TXIMLB</div>  
  Memo: `2967141893`

- **Litecoin (LTC)**:  
  <div class="wrap">ltc1qklestxa5shsym0gmuqmv2xewp56cst58vmhggl</div>

- **Tezos (XTZ)**:  
  <div class="wrap">tz1guFykj1dQAyiGH7g5YJVZzaGdoTWeMK81</div>

---

## 💡 Wallets
1. **Coinbase Wallet**: <div class="wrap">0x30D47A5815D94040291a819B8E39765AA09d44A8</div>  
2. **Metamask Wallet**: <div class="wrap">0x30D47A5815D94040291a819B8E39765AA09d44A8</div>  
3. **VeWorld Wallet**: <div class="wrap">0x020a79559990145e2f7d48c5771b233399b30bee</div>  
4. **Anchor Wallet**: `artabilly.gm`

---

## 🔗 Explore the Food4Thoth Hub

- 🌟 [FOOD4THOTH Website](../index.html)  
- 🌟 [FOOD4THOTH Instagram](https://www.instagram.com/emerald_path_food4th0th/profilecard/?igsh=dTJnejRlczhqNjho)  
- 🌟 [FOOD4THOTH Facebook](https://www.facebook.com/share/W8VnfAM2NHBAMTUb/?mibextid=JRoKGi)  
- 🌟 [Learn About ARTABILLIES](../Artabillies/index.html)  
- 🌟 [ARTABILLIES Website](http://www.artabillies.com)  
- 🌟 [ARTABILLIES Instagram](https://www.instagram.com/artabillies/profilecard/?igsh=MW1zbGg2Y2Z1a3FhdQ==)  
- 🌟 [ARTABILLIES Facebook](https://www.facebook.com/share/sEUxePbaAo9kyRNN/?mibextid=JRoKGi)  
- 🌟 [ARTABILLIES Facebook Group](https://www.facebook.com/share/g/6N5MX3W8pS3dbQuD/?mibextid=K35XfP)  
- 🌟 [Rstory, FOOD4THOTH & ARTABILLIES](../RstoryArtabillies/index.html)  
- 🌟 [Donations Page](../Donations/index.html)

---

## 💌 Contact

For inquiries or feedback: **[food4thoth@proton.me](mailto:food4thoth@proton.me)**

---

## 🌟 Join the Vision

**Food4Thoth** is a collaborative garden of research, tools, and art.  
Thank you to everyone who helps this project grow.

🌐 https://www.food4thoth.com/
