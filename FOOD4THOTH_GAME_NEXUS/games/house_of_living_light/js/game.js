(() => {
  ...
  function resizeCanvas() { const rect = $('#stage').getBoundingClientRect(); canvas.width = rect.width * devicePixelRatio; canvas.height = rect.height * devicePixelRatio; }
  function startFx() {
    ...
    function loop() {
      if (!state.settings.reduceMotion) {
        ...
        particles.forEach(p => { p.y -= p.s * (1 + state.chapter * .15); if (p.y < -0.02) { p.y = 1.02; p.x = Math.random(); } ctx.beginPath(); ctx.fillStyle = `hsla(${(p.h + state.chapter * 25) % 360},100%,70%,.38)`; ctx.arc(p.x*w, p.y*h, p.r*devicePixelRatio, 0, Math.PI*2); ctx.fill(); });
      }
