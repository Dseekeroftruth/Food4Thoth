...
const cardData = [
  ...
  { id: 77, name: "King of Pentacles", description: "The King of Pentacles symbolizes wealth, leadership, and mastery over the material realm.", imagePath: "./images/KINGPENTACLES.PNG", fileName: "./full_descriptions/78_14KingofPentaclesMinorArcana.txt" }
    ...
    var backImg = "<img class='back' src='./images/Place_Holder1.PNG' />";

    // Create deck array for random draws
    function createDeck() {
      deckArr = [];
      var id = 1;
      ...
      // Ranks in numeric form
      for (var s = 0; s < suits.length; s++) {
        for (var r = 1; r <= 14; r++) {
          var rankName = "";
          switch (r) {
            case 1: rankName = "ace"; break;
            case 11: rankName = "page"; break;
            case 12: rankName = "knight"; break;
            case 13: rankName = "queen"; break;
            case 14: rankName = "king"; break;
            default: rankName = r.toString();
          }
          var cardName = rankName + "_" + suits[s];
          deck[id] = { name: cardName, displayName: "..." };
          id++;
        }
      }

      // Major Arcana (in original logic, FOOL ~ 57)
      ...
      for (var m = 0; m < majorArcanaNames.length; m++) {
        deck[id] = { name: majorArcanaNames[m], displayName: "..." };
        id++;
      }

      // Put IDs into deckArr
      for (var i = 1; i <= 78; i++) {
        deckArr.push(i);
      }
    }

    // Return jQuery img for a deck index
    function getCardImage(cardIndex) {
      var cName = deck[cardIndex].name; 
      // For demonstration: map "ace_wands" -> "ACEWANDS.PNG"
      var fileName = cName.toUpperCase().replace(/_/g, "") + ".PNG";
      var $img = $("<img class='front'/>")
        .attr("src", "./images/" + fileName)
        .attr("alt", deck[cardIndex].name);
      return $img;
    }

    // Select random card from deckArr
    function randGen() {
  ...
  $("#td-1, #td-2, #td-3").empty().removeClass("invert");
  $("#td-display-name-1, #td-display-name-2, #td-display-name-3").empty();
  $("#rev-1, #rev-2, #rev-3").empty();

  $("#reading-btn").text("Another Reading?");
  
  for (var b = 1; b <= 3; b++) {
    var randIndex = randGen() - 1;  // Adjust index to match cardData array (0-77)

    if (randIndex >= 0 && randIndex < cardData.length) {
      var card = cardData[randIndex];
      var $cardImg = $("<img class='front'/>")
        .attr("src", card.imagePath)
        .attr("alt", card.name);

      if (Math.random() < 0.15) {
        $("#td-" + b).append($cardImg).addClass("invert");
        $("#td-display-name-" + b).html(`<p>${card.name}</p>`);
        $("#rev-" + b).html("<p><i>Reversed</i></p>");
      } else {
        $("#td-" + b).append($cardImg);
        $("#td-display-name-" + b).html(`<p>${card.name}</p>`);
      }

      $(`#rev-${b}`).append(`<p>${card.description}</p>`);
    } else {
