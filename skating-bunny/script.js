...
export class App {
  constructor() {
    ...
    this.gltfFile = "./rabbit6.glb";
    this.loadAssets();
  }

  loadAssets() {
    const loaderModel = new window.GLTFLoader();
    loaderModel.load(this.gltfFile, gltf => {
      this.model = gltf.scene;
      this.setUpScene();
    }, undefined, (err) => console.error("GLTF Loading Error:", err));
  }
  ...
  modifyFloorShader() {
    ...
    // Set uniforms explicitly to avoid cloning render target textures
  const uniforms = {
    ...THREE.UniformsLib['common'],
    ...THREE.UniformsLib['shadowmap'],
    ...THREE.UniformsLib['lights'],
    color: { value: new THREE.Color(this.bgrColor) },
    tDiffuse: { value: this.floor.getRenderTarget().texture }, // explicitly set the texture
    textureMatrix: { value: this.floor.material.uniforms.textureMatrix.value },
    tScratches: { value: this.bufferSim.output.texture }  // Your custom simulation texture
  };
